...
import {
  ...
  FiEye, FiDownload, FiCode, FiX, FiImage, FiEdit3, FiCalendar, FiTerminal,
  FiSun, FiMoon, FiXCircle
} from 'react-icons/fi';
...
<body>
  ...
  const renderPreviewModal = () => {
    ...
    const isBlocked = previewData.visibility === false;
        ...
        <div className="Gallery_modalContent" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="Gallery_modalBody">
            <div className="Gallery_modalResponsiveWrapper">
              {isBlocked ? (
                <div className="Gallery_blockedModalBody">
                  <FiXCircle className="blockedIconLarge" />
                  <h3>Access Restricted</h3>
                  <p>This entry has been hidden by an administrator for policy reasons. If you believe this is an error, please contact support.</p>
                </div>
              ) : (
          ...
          <div className="Gallery_modalFooter">
            <div className="Gallery_codeIndicator">
              {!isBlocked && previewData.html && <span className="badge">HTML</span>}
              {!isBlocked && previewData.css && <span className="badge">CSS</span>}
              {!isBlocked && previewData.js && <span className="badge">JS</span>}
              {isBlocked && <span className="badge blocked">Moderated</span>}
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`CssUpload_wrapper ${theme === 'dark' ? 'CssUpload_dark' : 'CssUpload_light'} ${viewMode === 'gallery' ? 'CssUpload_wrapper_gallery' : (todaysUploads.length > 0 ? 'CssUpload_wrapper_wide' : '')}`}>
      {renderPreviewModal()}
      ...
      {viewMode === 'upload' && (
        <div className={`CssUpload_formContainer ${todaysUploads.length > 0 ? 'CssUpload_hasTodays' : ''} auth_form-fade`}>
          ...
          <div className="CssUpload_uploadLeft">
            <div className="CssUpload_headerWrapper" style={{ marginBottom: '30px', borderBottom: '2px solid var(--cu-border)', paddingBottom: '15px' }}>
              <h2 className="CssUpload_header" style={{ margin: 0, border: 'none', padding: 0 }}>CSS Battle Uploads</h2>
            </div>
            ...
            <form onSubmit={handleSubmit} className="CssUpload_form">
              ...
              <div className="CssUpload_inputGroup">
                <label className="CssUpload_label">Description (Optional)</label>
                <textarea
                  className="CssUpload_textarea"
                  placeholder="Describe Target, approach or notes..."
                  value={description}
      ...
      {viewMode === 'gallery' && (
        <div className={`Gallery_container ${theme === 'dark' ? 'GAL_dark' : 'GAL_light'}`} style={{ padding: 0, overflow: 'visible', maxHeight: 'none', background: 'transparent' }}>
          ...
          {loadingGallery ? (
            ...
            <div style={{ textAlign: 'center', padding: '50px', background: 'var(--cu-panel)', borderRadius: '15px' }}>
              <h3 style={{ color: 'var(--cu-text-muted)' }}>No battles found for this filter.</h3>
            </div>
          ) : (
                ...
                <div key={dateStr} className="CssUpload_dateGroup">
                  ...
                  <div className="Gallery_grid">
                    {groupedUploads[dateStr].map((upload, idx) => (
                      <div key={idx} className={`Gallery_card Gallery_appear ${upload.visibility === false ? 'locked' : 'completed'}`}>
                        <div className="Gallery_cardScaler">
                          <div className="Gallery_cardPreviewArea">
                            {upload.visibility === false ? (
                              <div className="Gallery_statusIcon blocked" onClick={() => setPreviewData(upload)} style={{ cursor: 'pointer' }}><FiX /> <p>Admin Blocked</p></div>
                            ) : (
                          ...
                          <div className="Gallery_cardFooter">
                            <div>
                              ...
                              <div style={{ fontSize: '0.75rem', color: 'var(--gal-text-muted)', marginTop: '3px' }}>
                            ...
                            {upload.visibility !== false ? (
                              <div className="Gallery_actionIcons">
                                <button className="Gallery_iconBtn" onClick={() => {
                                  window.dispatchEvent(new CustomEvent('navigateToPlayground', {
                                    detail: { html: upload.html, css: upload.css, js: upload.js, isUploaded: true }
                                  }));
                              ...
                              <div className="Gallery_blockedNote">Blocked</div>
                            )}
                          </div>
                        </div>
                      </div>
