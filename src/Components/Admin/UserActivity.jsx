...
const UserActivity = () => {
  ...
  const [renderedContentCss, setRenderedContentCss] = useState('');

  const handleLivePreview = async (act) => {
    setShowPreview(act);
    setIsRenderLoading(true);
    setRenderedContentHtml('');
    setRenderedContentCss('');
    
    try {
      const fetchFile = async (url) => { if(!url) return ''; const r = await fetch(url); return await r.text(); };
      const [html, css] = await Promise.all([fetchFile(act.html_file), fetchFile(act.css_file)]);
      
      setRenderedContentHtml(html);
      setRenderedContentCss(css);
    } catch (e) {
      console.error(`Failed to fetch content for ${act.id}`, e);
    } finally {
  ...
  return (
    <div className={`Admin_container ${theme === 'dark' ? 'Admin_dark' : 'Admin_light'}`}>
      ...
      {showPreview && (
        <div className="Admin_modalOverlay" onClick={() => setShowPreview(null)}>
          <div className="Admin_galleryModal" onClick={(e) => e.stopPropagation()}>
            <div className="Admin_galleryHeader">
              <div className="Admin_galleryTitleGroup">
                 <span className={`Admin_sourceTag ${showPreview.type === 'cssBattles' ? 'battle' : 'task'}`}>
                    {showPreview.type === 'cssBattles' ? 'Battle' : 'Task'}
                 </span>
                 <h3 className="Admin_galleryTitle">{showPreview.battle_name || showPreview.description}</h3>
              </div>
              <button className="Admin_galleryClose" onClick={() => setShowPreview(null)}><FiX /></button>
            </div>

            <div className="Admin_galleryBody">
              {isRenderLoading ? (
                <div className="Admin_galleryLoader">
                  <div className="Admin_loaderPulse mini"></div>
                </div>
              ) : (
                <div className="Admin_galleryViewport">
                   {/* Scoped CSS for the preview */}
                   <style>{renderedContentCss}</style>
                   <div 
                     className="Admin_galleryContent" 
            ...
            <div className="Admin_galleryFooter">
               <div className="Admin_galleryAuthor">
                  <FiUser /> <span>{showPreview.users_profile?.email}</span>
               </div>
               <div className="Admin_galleryTime">
                  <FiClock /> <span>{new Date(showPreview.created_at).toLocaleDateString()}</span>
               </div>
            </div>
          </div>
        </div>
