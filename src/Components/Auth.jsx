...
const Auth = () => {
  ...
  useEffect(() => {
    const handleThemeChange = () => setTheme(localStorage.getItem('app-theme') || 'light');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        localStorage.removeItem('userRole'); // Clear potentially stale role
        localStorage.removeItem('dash_active_tab'); // Reset dashboard view

        // --- CHECK IF USER IS BLOCKED & RESOLVE ROLE ---
        ...
        const verifiedRole = profile?.role?.toLowerCase() || 'user';
        ...
        setMessage({ type: 'success', text: 'Login Successful! Re-verifying administrative privileges...' });
        
        // Final Failsafe: Use a short timeout followed by navigation to ensure state is settled
        setTimeout(() => {
          navigate('/dashboard');
          // Atomic reload to force refresh the entire permission tree
  ...
  return (
    <div className={`Signup_container ${theme === 'dark' ? 'Signup_containerDark' : ''} ${isAnimate ? 'auth_animated' : ''}`}>
      <div className="Signup_infoSection">
        <div className="Signup_overlay">
          ...
          <div className="Signup_singleImageWrapper">
            <img
              src={`https://picsum.photos/400/400?random=${mode === 'login' ? '1' : '5'}`}
              alt="Visual"
              className="Signup_image auth_pop-element"
            />
          </div>
        </div>
      </div>
